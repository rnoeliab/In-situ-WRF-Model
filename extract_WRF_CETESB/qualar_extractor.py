...
def all_photo(OUTPUT_CETESB,cetesb_login, cetesb_password, start_date, end_date, station, csv_photo=False):
    o3 = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 63, station)
    no = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 17, station)
    no2 = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 15, station)
    co = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 16, station)
    pm25 = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 57, station)
    pm10 = cetesb_data_download(OUTPUT_CETESB, cetesb_login, cetesb_password, start_date, end_date, 12, station)
    ...
    if csv_photo:
        all_photo_df.to_csv(OUTPUT_CETESB+'all_photo_' + str(station) + '.csv',
                            index_label='date')
    else:
        return all_photo_df
...
start_date = '01/01/2017'    # %d/%m/%Y
end_date = '31/12/2017'


for i in cetesb_stations.code:
    print('Downloading pol ' + cetesb_stations.name[cetesb_stations.code == i].values + ' Station')
    all_photo(OUTPUT_CETESB,cetesb_login, cetesb_password,
                        start_date, end_date, i, csv_photo=True)
    print('Downloading met ' + cetesb_stations.name[cetesb_stations.code == i].values + ' Station')
    all_met(OUTPUT_CETESB,cetesb_login, cetesb_password,
